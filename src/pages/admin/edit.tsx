...
const Edit = () => {
  ...
  return (
    <div className="flex flex-col items-center w-full h-full min-h-screen mid-complex-gradient">
      {hide && (
        <div className="flex fixed z-30 justify-center items-center w-full min-h-screen">
          <motion.div
            initial={{ scale: 0.1 }}
              ...
              {editData.index < 998 ? (
                <>
                  <div className="flex items-center space-x-2">
                    ...
                    <input
                      ...
                      className="px-2 h-8 bg-white bg-opacity-30 rounded-md border border-white appearance-none"
                    />
                  </div>
                  <div className="flex items-center space-x-2">
                ...
                <div className="flex flex-col items-start space-y-2">
                  ...
                  <textarea
                    ...
                    rows={6}
                    value={editData.title}
                    className="px-2 py-1 w-72 h-28 bg-white bg-opacity-30 rounded-md border border-white appearance-none"
                  />
                </div>
              )}
            </div>
        ...
        <div className="flex flex-col w-full rounded-3xl item-center round-lg glass-panel">
          <div className="flex flex-col items-center mx-6 mt-8 h-full md:mt-12 md:mx-10">
            ...
            <div className="relative mt-3 md:mt-6">
              <h1 className="text-2xl font-extrabold text-center text-purple-625 md:text-3xl">
                {pageData.title}
              </h1>
              <PencilIcon
                onClick={() => {
            ...
            <p className="text-xs font-medium text-center md:text-base text-purple-625">
              {pageData.description}
              <PencilIcon
                onClick={() => {
                  edit(998, pageData.description, "");
                }}
                className="inline absolute mt-1 ml-1 w-3 h-3 text-blue-600 md:w-4 md:h-4"
              />
            </p>
            ...
            <motion.div
              ...
              className="py-3 px-3 mt-6 bg-white bg-opacity-50 rounded-full cursor-pointer"
            >
              <PlusIcon className="w-6 h-6 text-blue-600" />
            </motion.div>

            <div className="px-1 pb-5 mt-4 w-full h-2/5 text-xs font-semibold overflow-custom md:h-2/4 md:mt-6 md:text-base font-display text-purple-625">
              {content}
            </div>
            ...
            <motion.div
              ...
              whileHover={{ scale: 1.1 }}
              className="py-3 px-3 mb-6 bg-white bg-opacity-50 rounded-full cursor-pointer"
            >
              <PlusIcon className="w-6 h-6 text-blue-600" />
            </motion.div>
          </div>
      ...
      <img
        className="z-10 mt-10 mb-8 w-44 md:mt-8 md:w-64"
